...
import { Grid, Paper, Text, ActionIcon, Group, Modal, Container } from "@mantine/core";
...
function QuickAccessGrid({ habits, onTogglePin, onLog }: QuickAccessGridProps) {
  ...
  const pinnedHabits = habits.filter((h) => h.isPinned).sort((a, b) => (a.order || 0) - (b.order || 0));
  ...
  return (
    <>
      <Container fluid m={0} p={0}>
        <Grid gutter="md" m={0}>
          {pinnedHabits.map((habit) => (
            <Grid.Col key={habit._id} m={0} span={{ base: 12, sm: 6, md: 4, lg: 3 }}>
              <Paper
                p="md"
                withBorder
      ...
      {/* Logging Modal */}
      <Modal opened={!!selectedHabit} onClose={() => setSelectedHabit(null)} size="lg" title={selectedHabit?.question}>
        {selectedHabit && (
          <HabitCard
            habit={selectedHabit}
            onLog={(log) => {
